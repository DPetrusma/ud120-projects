...
# I've chosen 100 splits so that it finished on my machine. The other parameters match the tester function
sss = StratifiedShuffleSplit(n_splits = 100, test_size = 0.1, random_state = 42)

# For the pipeline I am trying a random forest here, and I have included a Min Max Scaler and a PCA
estimators = [("Scale", MinMaxScaler()), ("Reduce_Dimension", PCA(svd_solver = "randomized")), ("Classifier", RandomForestClassifier())]

# Create the pipeline
pipe = Pipeline(estimators)

# These are the parameters to loop through for the different classifiers
params = {  "Reduce_Dimension__n_components" : [5, 8, 9, 10, 11, 12, 13, 14, 15],
